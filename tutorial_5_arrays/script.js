...
restaurantList.appendChild(restaurantDiv);
    ...
    cheapRestaurants.forEach(function(restaurant) {
        ...
        price.textContent = `Price: ${restaurant.priceRange}`;
    ...
    if (bestRestaurant) {
        const div = document.createElement('div');
        div.classList.add('found-restaurant');

        const name = document.createElement('div');
        name.classList.add('restaurant-name');
        name.textContent = bestRestaurant.name;

        const cuisine = document.createElement('div');
        cuisine.classList.add('restaurant-cuisine');
        cuisine.textContent = `Cuisine: ${bestRestaurant.cuisine}`;

        const rating = document.createElement('div');
        rating.classList.add('restaurant-rating');
        rating.textContent = `Rating: ${bestRestaurant.rating}`;

        div.appendChild(name);
        div.appendChild(cuisine);
        div.appendChild(rating);

        foundItem.appendChild(div);
    } else {
        foundItem.textContent = 'No restaurant found with rating 4.8.';
    }
