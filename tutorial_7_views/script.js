...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showCardView() {
        ...
        card.innerHTML = `
            <div class="card-name">${props.name || 'Unknown Restaurant'}</div>
            <div class="card-location">${props.city || 'Unknown City'}</div>
            <div class="card-status ${status}">
                ${status === 'compliant' ? 'Compliant' : 
                  status === 'non-compliant' ? 'Non-Compliant' : 'No Data'}
            </div>
    ...
    function showTableView() {
        ...
        row.innerHTML = `
            <td class="table-restaurant-name">${props.name || 'Unknown'}</td>
            <td>${props.city || 'Unknown'}</td>
            <td>${formatDate(props.inspection_date)}</td>
            <td><span class="table-status ${status}">
                ${status === 'compliant' ? 'Compliant' : 
                  status === 'non-compliant' ? 'Non-Compliant' : 'No Data'}
            </span></td>
            <td class="compliance-indicator ${props.handwashing === 'In Compliance' ? 'pass' : 'fail'}">
                ${getComplianceIndicator(props.handwashing)}
            </td>
            <td class="compliance-indicator ${props.food_temperature === 'In Compliance' ? 'pass' : 'fail'}">
                ${getComplianceIndicator(props.food_temperature)}
            </td>
        `;
    ...
    const compliantCount = restaurants.filter(r => 
        getComplianceStatus(r) === 'compliant'
    ).length;
    
    const nonCompliantCount = restaurants.filter(r => 
        getComplianceStatus(r) === 'non-compliant'
    ).length;
    
    const complianceRate = ((compliantCount / totalRestaurants) * 100).toFixed(1);
    
    const uniqueCities = [...new Set(restaurants.map(r => r.properties.city))].length;
    
    // Create stat cards
    const stats = [
        { label: 'Total Restaurants', value: totalRestaurants },
    ...
    restaurants.forEach(function(restaurant) {
        const city = restaurant.properties.city || 'Unknown';
        
        if (!cityData[city]) {
            cityData[city] = { total: 0, compliant: 0 };
        }
        
        cityData[city].total++;
        if (getComplianceStatus(restaurant) === 'compliant') {
            cityData[city].compliant++;
        }
    });
    
    // Sort cities by total restaurants and show top 10
    const sortedCities = Object.entries(cityData)
        .sort(([,a], [,b]) => b.total - a.total)
        .slice(0, 10);
    
    sortedCities.forEach(function([city, data]) {
        const rate = ((data.compliant / data.total) * 100).toFixed(1);
        
        const cityCard = document.createElement('div');
        cityCard.className = 'city-stat';
        cityCard.innerHTML = `
            <div>
                <div class="city-name">${city}</div>
                <div class="city-compliance">${data.total} restaurants</div>
            </div>
            <div class="city-compliance">${rate}% compliant</div>
        `;
        cityStats.appendChild(cityCard);
    });
